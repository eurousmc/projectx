    ...
    while ($row = mysqli_fetch_array($rs)) {
      if ($_SESSION['user_login_name'] != $row['Username']) {
        ...
        VALUES(NULL,
          ...
          '{$_SESSION['user_login_realname']} ได้เพิ่มเอกสาร $userfile ลงใน $project_name',
          '$project_id',
          '".date('Y-m-d')."',
          '".date('H:i')."',
          'Unread')";
...
//******************************************************************************
  ...
  header("location: edit_fileupload.php?project_id=$project_id&msg=เพิ่มเอกสารเรียบร้อย&page={$_GET['page']}");
} else {
  header("location: edit_fileupload.php?project_id=$project_id&msg=โปรดเลือกเอกสาร&page={$_GET['page']}");
}
// end upload's code ****************************************************************
